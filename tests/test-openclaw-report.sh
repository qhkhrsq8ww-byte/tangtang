...
echo "$feat" | grep -q "./cat.sh reactions" || bad "features missing reactions"
echo "$feat" | grep -q "今日休息四步" || bad "features missing today four steps"
ok "features lists openclaw"

hw="$("$CAT/cat.sh" hwcheck 2>/dev/null)" || bad "hwcheck exit"
echo "$hw" | /usr/bin/python3 -c '
import json,sys
d=json.load(sys.stdin)
assert "os" in d and "ffmpeg" in d
assert "transcript" not in json.dumps(d)
' || bad "hwcheck json"
ok "hwcheck"

tr="$("$CAT/cat.sh" today-report hanghang 2>/dev/null)" || bad "today-report exit"
echo "$tr" | grep -q "=== today-report ===" || bad "today-report banner"
echo "$tr" | grep -q "transcript" && bad "today-report leaked transcript"
ok "today-report labels only"
